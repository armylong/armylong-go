...
func TestBug1_TransferConcurrency(t *testing.T) {
	...
	// 用户1充值100
	err := YangfenBusiness.Recharge(ctx, uid1, 100)
	if err != nil {
		t.Fatalf("充值失败: %v", err)
	}
...
// 3. 手动设置余额为100（模拟用户重新获得积分）
// 4. 消费50
// 5. 退款
// 6. 实际：退款成功，余额增加50
// 7. 问题：这笔积分已经过期了，退款应该失败或特殊处理
func TestBug2_RefundAfterExpire(t *testing.T) {
	...
	// 手动设置余额为100（模拟用户通过其他方式获得积分）
	webcache.RedisClient.Set(ctx, YangfenBalanceKey+uid1, "100", 0)
	balance, _ = YangfenBusiness.GetBalance(ctx, uid1)
	t.Logf("手动设置余额后: %d", balance)
	...
	// Bug: 退款成功，但原始积分已过期，这里存在业务逻辑漏洞
	t.Log("Bug复现! 退款成功，但原始积分已过期，退款逻辑未考虑过期情况")
}
...
func TestBug3_ConsumeBonusNotApplied(t *testing.T) {
	...
	// 用户1充值200
	err := YangfenBusiness.Recharge(ctx, uid1, 200)
	if err != nil {
		t.Fatalf("充值失败: %v", err)
	}
...
func TestBug4_TransactionNotClearedAfterExpire(t *testing.T) {
	...
	err = YangfenBusiness.Recharge(ctx, uid1, 0)
	if err == nil {
		t.Log("充值0触发过期检查")
	}
